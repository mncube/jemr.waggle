## code to prepare `matdat_loc_r1_v2` dataset goes here
devtools::load_all()
library(Matching)
library(rgenoud)
...
for (g in grades) {
  ...
  matched_grade <- matchup(grade_data, .form = form, ratio = 1,
                           ...
                           m.order = "smallest",
                           pop.size = 200, max.generations=200)
...
# Combine results
matdat_loc_r1_v2 <- dplyr::bind_rows(matched_list)
usethis::use_data(matdat_loc_r1_v2, overwrite = TRUE)
